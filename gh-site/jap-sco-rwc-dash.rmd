...
posix_KO <- as.POSIXct("2019-10-13 10:15:00", tz="UTC")
posix_FT <- as.POSIXct("2019-10-13 13:15:00", tz="UTC")
ref_regex <- "( ref|O\'Keeffe|O\'Keef|Ben|Okeef)"
match_data <- jsonlite::fromJSON('../data/reddit-match-threads/dh87gm_matchdata.json')
mt_comms <- fread('../data/reddit-match-threads/dh87gm_final.csv', encoding = 'UTF-8')
timeline <- fread('../data/reddit-match-threads/dh87gm_timeline.csv')
...
```{r "high"}
reddit_comment(as.list(mt_comms[plaintext != "[removed]",][which.max(score_praw),][1]))
```

### Lowest Score {.flex-chart}
```{r "low"}
reddit_comment(as.list(mt_comms[plaintext != "[removed]",][which.min(score_praw),][1]))
```
