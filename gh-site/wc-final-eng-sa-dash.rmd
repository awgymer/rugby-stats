...
posix_KO <- as.POSIXct("2019-11-02 08:30:00", tz="UTC")
posix_FT <- as.POSIXct("2019-11-02 11:30:00", tz="UTC")
ref_regex <- '( ref|Garces|garces|jerome|Jerome|Jérôme|Garcès|jérôme|garcès)'
match_data <- jsonlite::fromJSON('../data/wc_final_match.json')

mt_comms <- fread('../data/reddit-match-threads/dqcos5_final.csv', encoding = 'UTF-8')
timeline <- fread('../data/wc_final_timeline.csv')
...
timeline[team_id == 34, event_team:=TEAM_0_NAME]
timeline[team_id == 39, event_team:=TEAM_1_NAME]
timeline[is.na(event_team), event_team:='']
timeline[,graph_lab := glue_data(.SD, '{sprintf("%02d", match_time%/%60)}:{sprintf("%02d", match_time%%60)}<br>{manual_label} {titleize(event_team)}<br>{score0} - {score1}')]
...
div(
  ...
  span(class = "match-facts-header",
    span(class = "match-facts-team0",
      ...
      img(
        src=get_team_flag(tolower(TEAM_0_NAME), team_cols=team_cols, path="team-colours"), 
    ...
    span(class = "match-facts-team1",
      img(
        src=get_team_flag(tolower(TEAM_1_NAME), team_cols=team_cols, path="team-colours"), 
...
mt_comms[,
  .(
    ...
    tot_comments=.N, 
    flag=glue_data(.SD, '<img src="{sapply(tolower(flair_country), get_team_flag, team_cols=team_cols, path="team-colours")}" title="{titleize(flair_country)}" class="flag table-flag">')),
  by=flair_country
][order(-tot_comments)] %>% 
...
plot_ly(
  ...
  layout(
    ...
    xaxis = list(title = 'Times Used', fixedrange = T),
    dragmode = 'pan',
    modebar = list(orientation = 'v')
  )
```
